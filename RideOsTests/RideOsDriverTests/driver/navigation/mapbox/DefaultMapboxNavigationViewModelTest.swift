...
class DefaultMapboxNavigationViewModelTest: ReactiveTestCase {
    ...
    private var viewModelUnderTest: DefaultMapboxNavigationViewModel!
    ...
    override func setUp() {
        ...
        recordingDirectionsInteractor =
            RecordingMapboxDirectionsInteractor(directions: DefaultMapboxNavigationViewModelTest.directions)

        let deviceLocator = FixedDeviceLocator(deviceLocation: DefaultMapboxNavigationViewModelTest.deviceLocation)
        let schedulerProvider = TestSchedulerProvider(scheduler: scheduler)
        viewModelUnderTest = DefaultMapboxNavigationViewModel(deviceLocator: deviceLocator,
    ...
    func testViewModelDoesNotIndicateItShouldHandleReroutes() {
        XCTAssertFalse(viewModelUnderTest.shouldHandleReroutes)
    }

    func testViewModelUpdatesDirectionsAfterRouteFromOriginToDestinationRequest() {
        scheduler.scheduleAt(0) {
            self.viewModelUnderTest.route(
                from: DefaultMapboxNavigationViewModelTest.deviceLocation.coordinate,
                to: DefaultMapboxNavigationViewModelTest.destination.coordinate
            )
        }
        ...
        XCTAssertEqual(directionsRecorder.events, [
            next(1, DefaultMapboxNavigationViewModelTest.directions),
        ])
    ...
    func testViewModelUpdatesDirectionsAfterRouteToDestinationRequest() {
        scheduler.scheduleAt(0) {
            self.viewModelUnderTest.route(to: DefaultMapboxNavigationViewModelTest.destination.coordinate)
        }
